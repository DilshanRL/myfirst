...
fun_00() {
    for word in "${count3[@]}"; do
                    ...
                    #create a data_file
                    awk 'NF > 1' /opt/aepona/ngw/gwuser/data_para/data"$word".txt > /opt/aepona/ngw/gwuser/para/para"$word".txt 



                    #print the parameter and the date
                    echo -e "${green}${DATE[D1]} $word${reset}"


                    # Read the data from the file
                    data_file=/opt/aepona/ngw/gwuser/para/para"$word".txt 
                    data=$(cat "$data_file")
    ...
    grep "${file_type[count2]}" "${DATE[D1]}" > /opt/aepona/ngw/gwuser/grp/srv1
        ...
        do 
         ...
         for item in "${count[@]}"
          ...
          echo -e "${yellow}24.${reset} All"
          echo -e "${yellow}25.${reset} Go Back"
        ...
        do 
         ...
         for item in "${count3[@]}"
          ...
          i=$((i+1))
            ...
            echo -e "${yellow}13.${reset} All"
            echo -e "${yellow}14.${reset} Go back"
